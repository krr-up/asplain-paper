...
defined in \cite{ly:iclp,ly:spie,trieu2022xasp} or 
...
Given a program $P$ and an answer set $M$ of $P$,  
for an atom $x \in M$, let $T_x = (V_x, E_x)$ be the subgraph of $MG^M_P$ 
that explains $x$ as defined in the previous section.   
...
\begin{itemize} 
    ...
    \item for $i > 1$, let $x \in V_{i} \setminus (V_{i-1} \cup \Omega)$,   
      \begin{itemize} 
        \item if $x \in V^+_{i+1}$, then let $T_x = (V_x, E_x)$ be the subgraph of $MG^M_P$ 
        ...
        \item if $x \in V^-_{i+1}$, then let $R(x) = \{r \mid r \in P, x \in H(r), M
          \not\models B(r)\}$. 
        ...
        %   $y \in B^+(r)$ such that $y \not\in M$ or $y \in B^-(r)$ such that $y \in M$.
          ...
          or $y \in B^-(r) \cap M$. 
        \begin{itemize} 
            \item $V_{i+1}^+ = V_i^+ \cup (Y \cap \bigcup_{r \in R(x)} B^-(r))$, 
            \item $V_{i+1}^- = V_i^- \cup (Y \cap \bigcup_{r \in R(x)} B^+(r))$, and 
      ...
      \end{itemize} 
\end{itemize}
Because $P$ is finite, the construction terminates after finitely many steps. 
