...
\node[encoding, right=10mm of meta-tools, fill=styleyellow] (reify) {\texttt{reified.lp}};
...
\node[subsection, fit=(enc)(inst)(ass)(reify)(reifypg)(dyn)(constructpg)(pg)(abd),
  label=above:{%
    \raisebox{0.1ex}{\tikz[baseline=-0.3ex]\node[draw,circle,inner sep=1pt,fill=white]{1};}~%
    \raisebox{1ex}{Construct $\pg{\pr}$}
  }
] (pgbox) {};
\end{pgfonlayer}
...
\node[element, right=40mm of subg] (mgref) {$\mgpi{\pr}{\m_r}$ \\\texttt{node/2}\\\texttt{edge/3}};
\node[ above= 15mm of mgref, color=red] (unsat) {   No model for \pr};

\draw[subarrw] (model.east) -| (constructref);
\draw[subarrw] (pgforce.east) -| (constructref);
\draw[arrw] (subg) -| (constructref);
\draw[arrw] (constructref.east) -- ++(0.5,0) |-  node[below, font=\tiny]{SAT} (mgref);
\draw[arrw, color=red] (constructref.east) -- ++(0.5,0) |- node[above, font=\tiny]{UNSAT} (unsat.west);

\begin{pgfonlayer}{sections}
\node[subsection, fit=(model)(pgforce)(subg)(constructref)(mgref)(unsat),
  label=above:{%
    \raisebox{0.1ex}{\tikz[baseline=-0.3ex]\node[draw,circle,inner sep=1pt,fill=white]{2};}~%
    \raisebox{1ex}{Construct$\mgpi{\pr}{\m_r}$}
  }
] (refbox) {};
\end{pgfonlayer}
...
\node[ below=10mm of pgfoil, color=red] (noex) {No foil};
...
\draw[arrw] (clingofoil.east) -- ++(0.5,0) |- node[above, font=\tiny]{SAT} (pgfoil);
\draw[arrw, color=red] (clingofoil.east) -- ++(0.5,0) |- node[below, font=\tiny]{UNSAT} (noex.west);

% \draw[arrw] (clingofoil) -- (mgfoil);

\begin{pgfonlayer}{sections}
\node[subsection, fit=(subgfoil)(constructfoil)(query)(cost)(costfn)(clingofoil)(pgfoil),
label=above:{%
    \raisebox{0.1ex}{\tikz[baseline=-0.3ex]\node[draw,circle,inner sep=1pt,fill=white]{3};}~%
    \raisebox{1ex}{Construct $\bestfoils(\pr, \addable, \removable, Q, \foilcost)$}
  }
] (foilbox) {};
...
\begin{pgfonlayer}{sections}
\node[subsection, fit=(contrastenc)(contrast)(cg),
  label=above:{%
    \raisebox{0.1ex}{\tikz[baseline=-0.3ex]\node[draw,circle,inner sep=1pt,fill=white]{4};}~%
    \raisebox{1ex}{Construct $\cg{\pr}{\m_r}{\pf}{\m_f}$}
  }
] (contrastbox) {};
\end{pgfonlayer}
