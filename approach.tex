...
\begin{example}[Program Graph]\label{ex:example1}
  ...
  \comment{In the examples we say poisoned and not paralysis. Which one is right?}
  ...
  %%
  $P_1$ has two models, namely $\m_1 = \{w, d, a\}$ and $\m_2 = \{h, d, p\}$,
  ...
  %
  Circle-nodes in graph $G_{P_{1}}$ represent atoms appearing in $P_1$, that is, the set $\Va = \{d,a,h,w,p\}$.
  %
  Rectangle-nodes in graph $G_{P_{1}}$ represent the body of some rule in $P_1$, in particular, sets $\Vc = \{1\}$
  and $\Vd = \{2,3,4,5,6\}$ correspond to the set of rules in $P_1$ whose head is a choice, respectively a disjunction.
  %
...
\begin{itemize}
  \item
  $\Vc$, $\Vd$, $\Va$ are pairwise disjoint \comment{S: This would be simpler to show (I think) if the sets for \isdisj{r} is rather Dis(P)}
  \item
  ...
  \item
  As an additional advantage, this graph is easy to generate using  clingo's metaprogramming features~\cite{kamrom23a}.
  That is, it resembles the meta program obtained by the grounder (gringo) from the original program.
  This is discussed in detail in the implementation section.
...
\begin{proposition}
  Given two labelled disyunctive, ground programs $P_1$ and $P_2$ and their respective program graphs $\pg{P_1}$ and $\pg{P_2}$, then
  \[ \pg{P_1} = \pg{P_2} \rightarrow P_1 = P_2 \]
\end{proposition}
...
\begin{figure}
  ...
  The entire program graph $G_{P_{1}} = \langle \Vc \cup \Vd \cup \Va, \Epos\cup \Eneg \rangle$ can be seen, but the model subgraph $\mgpi{P_1}{\m_1}$, which is highlighted  the answer set $\m_1 = \{w,a,d\}$.}
...
\begin{definition}[Valid Foil]\label{def:valid-foil}
  ...
  \begin{itemize}
    ...
    %
    \item a set of \emph{removable} labelled rules $\removable \subseteq \pr$ of the form \eqref{f:disjunctive-rule} and \eqref{f:choice-rule},
    %
    \item and
    %
    \item a set of \emph{addable} labelled rules $\addable$ of the form \eqref{f:disjunctive-rule} and \eqref{f:choice-rule},
    %
  \end{itemize}
  %
  ...
  \begin{itemize}
    ...
    \comment{Can we say $\m_f\models \pf$? \m think we do this in other papers}
  \end{itemize}
...
\begin{itemize}
  ...
  %
  \item {\color{blue} This definition does not impose the existence of a \emph{reference model} $\m_r \models \pr$ neither that $\m_r \nvDash Q$.} It's convenient for allowing particular scenarios:
  \begin{itemize}
    \item \emph{unsatisfiable reference programs}: $AS(\pr) = \emptyset$ and $Q = \emptyset$, that is, we only want to recover satisfiability; or (b), $Q \neq \emptyset$, we want it to be sat and force something.
    \item The reference and foil models may collide $\m_r = \m_f$, which means that we are in fact doing no contrast. In other words, $\m_r$ already satisfy the query.
  \end{itemize}
  %
  ...
  %
  \item The set $\removable$ (\emph{removables}) is only restricted to be a subset of $\pr$, while the set $\addable$ (\emph{addables}) is free, since we want to give full flexibility.
  %
  ...
  \begin{itemize}
    ...
    %
    \item The role of $\added$ and $\removed$ will play an important role when designing and implementing good explanation preferences.
  \end{itemize}
  %
  \item \textbf{Visualization:}
  \begin{itemize}
    \item We now have to handle the removals and the additions, in other words, we have to deal $\pr \neq \pf$.
    \item To do so, we use the program graph $\pg{\pr \cup \pf}$ as the base of the visualization.
    \item We highlight $\mgpi{\pf}{\m_f}$ as an explanation of the foil.
    \item Finally we also highlight the added and removed rules using the sets $\added$ and $\removed$.
...
\begin{example}[Removing rules for explaining a query]\label{ex:example2}
  ...
  %
  To that aim, let us consider rule $4$ (James have drank) as a \emph{removable rule} by setting $\removable = \{ 4 \}$.
  %
  In this case, we will not have any \emph{addable rule}, that is $\addable = \emptyset$.
  %
  To generate an explanation, we would take any valid foil $(\pf, \m_f) \in Foils(P2, \addable, \removable, Q)$ and compute the foil model subgraph $\mgpi{\pf}{\m_f}$.
  %
  For a nicer visualization, this model subgraph can be depicted over the program graph $\pg{\pr \cup \pf}$ as in Figure~\ref{fig:example2}.
  %
\end{example}
%
\begin{figure}
...
\begin{definition}[Foil cost function]\label{def:foil-cost}
  ...
  %
  \[
    \mathit{foil\_cost}_{\pr,\addable,\removable,Q} :
    \mathrm{Foils}(\pr, \addable, \removable, Q) \rightarrow \mathbb{N}.
  \]
...
\begin{definition}[Best foils]\label{def:best-foils}
  ...
  \begin{itemize}
    ...
    \item and a set of addable labelled rules $\addable$,
    %
    \item and a foil cost function $\mathit{foil\_cost}_{\pr,\addable,\removable,Q}$,
  \end{itemize}
  %
  the set of \emph{best foils} is defined as
  %
  \[
  \mathrm{BestFoils}(\pr, \addable, \removable, Q, \mathit{foil\_cost})
    = \operatorname*{arg\,min}_{(\pf, \m_f) \in \mathrm{Foils}(\pr, \addable, \removable, Q)}
      \mathit{foil\_cost}_{\pr,\addable,\removable,Q}(\pf, \m_f).
  \]
\end{definition}
...
\begin{example}[Selecting minimal foils using the program-difference cost]\label{ex:best-foils}
  ...
  Therefore,
  \[
  \mathrm{BestFoils}(\pr, \addable, \removable, Q, \pdiff)
    = \{(\pf, \m_f) \in \mathrm{Foils}(\pr, \addable, \removable, Q)
        \mid |\,\pr \triangle \pf\,| = 1\}.
  \]
  %
  Minimizing $\pdiff$ thus selects the left and middle foils from Figure~\ref{fig:example3-2}, which represent the \emph{cardinality}-minimal deviations from the reference program needed to recover satisfiability.
\end{example}
...
\begin{itemize}
  ...
  %
  \item \addable contrastive explanation compares this \emph{reference world} with an \emph{foil world} where the outcome differs.
  %
...
\begin{definition}[Contrastive Subgraph]
  ...
  \begin{itemize}
    ...
    %
    \item a program graph $\pg{\pr \cup \pf} = \langle \Vc, \Vd, \Va, \Epos, \Eneg \rangle$,
    \comment{
  ...
  \begin{itemize}
    ...
    \item $Econ_{+} = \{(v1, v2) \mid (v1, v2) \in \Epos \land v1,v2 \in Vcon\}$
    \item $Econ_{-} = \{(v1, v2) \mid (v1, v2) \in \Eneg \land v1,v2 \in Vcon\}$
    \item $Econ_{-} = \{ (a, \ell) \mid (a, \ell) \in \Eneg \land [ (\m_r \models a) \oplus (\m_f \models a)]\}$
  \end{itemize}
\end{definition}
%
\begin{proposition}
  ...
  \begin{itemize}
    \item the \emph{model graph} $\mgpi{\pr}{\m_r}$ is a subgraph of $\cg{\pr}{\pf}{\m_r}{\m_f}$
    \item the \emph{model graph} $\mgpi{\pf}{\m_f}$ is a subgraph of $\cg{\pr}{\pf}{\m_r}{\m_f}$
    \item $\cg{\pr}{\pf}{\m_r}{\m_f}$ is a subgraph of the \emph{program graph} $\pg{\pr \cup \pf}$.
  \end{itemize}
\end{proposition}
%
...
\begin{example}[Contrastive Explanations for a query]\label{ex:example4}
  ...
  %
  Imagine that the user is shown with a visualization of $\m_2 = \{h, d, p\}$ and wonders why James has been poisoned, that is, user expected James not to be poisoned.
  %
  Now, if we set $Q = \{ \neg p \}$,
  $\addable = \emptyset$
  and
  $\removable = \{ 4 \}$ (allowing to remove the fact $d$),
  %
  then for any $\mgpi{\pf}{\m_f} \in Foil(\pr, \addable, \removable, Q)$,
  %
  we can build a contrastive graph $\cg{\pr}{\pf}{\m_r}{\m_f}$.
  %
...
\begin{figure}
  ...
  \end{minipage}
  \caption{Possible contrastive explanations for the pair $(P1, \m_2)$ and $Q = \{ \neg p \}$, when $\addable = \emptyset$ and $\removable = \{ 4 \}$}
  \label{fig:example4}
\end{figure}
...
\begin{itemize}
  \item The reference model subgraph $\mgpi{\pr}{\m_r}$ is coloured in purple whereas the foil model subgraph $\mgpi{\pf}{\m_f}$ is coloured in green.
  \item Also, what is not part of $\mgpi{\pf}{\m_f}$, is shown with less opacity.
  \item The atom nodes, for any atom used in the query $Q$ are shown with a thicker border.
  \item As when visualizing foils (for instance, in Figures~\ref{fig:example3-1} and~\ref{fig:example3-2}), removals and additions (e.g. the $\removable' = \pr \setminus \pf$ and the $\addable' = \pf \setminus \pr$ sets) are shown in red and blue respectively.
  \item Finally, for those constraint rule nudes, that are existentially statisfied by $\m_r$ and $\m_f$ together, we colour them with the orange color.
...
\begin{definition}[Contrast cost function]\label{def:contrast-cost}
  ...
  %
  \[
    \mathit{contrast\_cost}_{(\pr, \m_r),\addable,\removable,Q} :
    \mathrm{Foils}(\pr, \addable, \removable, Q) \rightarrow \mathbb{N}.
  \]
...
\begin{definition}[Best constrast]\label{def:best-contrasts}
  ...
  \begin{itemize}
    ...
    \item and a set of addable labelled rules $\addable$,
    %
    \item and a contrast cost function $\mathit{contrast\_cost}_{(\pr, \m_r),\addable,\removable,Q}$,
  \end{itemize}
  %
  the set of \emph{best contrasts} is defined as
  %
  \[
  \mathrm{BestContrasts}((\pr, \m_r), \addable, \removable, Q, \mathit{contrast\_cost})
    = \operatorname*{arg\,min}_{(\pf, \m_f) \in \mathrm{Foils}(\pr, \addable, \removable, Q)}
      \mathit{contrast\_cost}_{(\pr, \m_r),\addable,\removable,Q}(\pf, \m_f).
  \]
\end{definition}
...
\begin{example}[Model-difference cost function]\label{ex:model-difference-cost-function}
  ...
  \[
    \mathit{MDiff}_{(\pr, \m_r),\addable,\removable,Q}(\pf, \m_f)
      = |\,\m_r \,\triangle\, \m_f\,|,
  \]
...
\begin{example}[Reference–Foil difference cost function]\label{ex:reference-foil-difference-cost-function}
  ...
  \[
    \mathit{RFDiff}_{(\pr, \m_r),\addable,\removable,Q}(\pf, \m_f)
      = \mathit{MDiff}_{(\pr, \m_r),\addable,\removable,Q}(\pf, \m_f)
      + \pdiff_{\pr,\addable,\removable,Q}(\pf, \m_f).
  \]
\end{example}
...
\begin{example}[Selecting the best contrastive explanations]\label{ex:best-contrastive-explanations}
  ...
  %
  First, if we use the $\pdiff$ foil cost function, only the foil corresponding to explanation (1) would be part of $\mathit{Bestfoils}(\pr, \addable, \removable, Q, \pdiff)$ with a weight of $0$.
  %
  ...
  %
  In order to notice this latter semantic change, we need to include the \emph{reference model} $\m_r$ as well in the preference by using \emph{contrast cost functions} instead.
  %

  %
  For instance, if we use the $\mathit{MDiff}$, which counts the differences among $\m_r$ and $\m_f$, is easy to see that contrast we obtain
  ...
  %
  Therefore $\mathrm{BestContrasts}((\pr, \m_f), \addable, \removable, Q, \mathit{MDiff})$ would only include the foil leading to (2) as the preffered explanation.
  %
