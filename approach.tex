...
%
Given a rule $r$ like \eqref{f:disjunctive-rule} or \eqref{f:choice-rule}, we denote its label as $\lbl{r}\eqdef \ell$,
%
and so, we denote the rule $r$ corresponding to a label $\ell$ as $\InvLb{\ell}$.
%
Finally, we denote $\Lbs{P} \eqdef \{ \lbl{r} \mid r \in P \}$ as the set of all labels of the rules in a labelled program $P$.
\medskip
...
\begin{itemize}
    \item $\Vd = \{ \lbl{r} \mid r \in P \land (\isdisj{r} \vee \iscons{r})      \}$ is the set of rules's labels, whose head is a dijunction;
    \item $\Vc = \{ \lbl{r} \mid r \in P \land \ischoice{r}    \}$ is the set of rules's labels, whose head is a choice;
...
\begin{itemize}
  \item $\forall r \in P, \forall a \in \BB{r}  \rightarrow (a,\lbl{r})  \in \Epos$
  \item $\forall r \in P, \forall a \in \BBn{r} \rightarrow (a, \lbl{r}) \in \Eneg$
  \item $\forall r \in P, \forall h \in \HH{r}  \rightarrow (\lbl{r}, h) \in \Epos$
\end{itemize}
%
\end{definition}
%
...
\begin{example}[Program Graph]
  ...
  \begin{figure}
    ...
    \caption{Program Graph $\pg{P_{1}}$ corresponding to program in Listing~\ref{lst:example1}. }
  \end{figure}\comment{What do we do with optional nodes, that is abduction...}
  %
  Circle-nodes in graph $\pg{P_{1}}$ represent atoms appearing in $P_1$, that is, the set $\Va = \{d,a,h,w,p\}$.
  %
  Rectangle-nodes in graph $\pg{P_{1}}$ represent the body of some rule in $P_1$, in particular, sets $\Vc = \{1\}$
...
\begin{proposition}
  Given two disyunctive, ground programs $P_1$ and $P_2$ and their respective program graphs $\pg{P_1}$ and $\pg{P_2}$, then
...
\begin{definition}[Model Subgraph from Model]
  ...
  and an answer set $I \subseteq \Va$ of $P$,
  a \emph{Model Subgraph} is a pair $M\pg{P}^I = \langle V, E \rangle$ such that
  \begin{itemize}
    % take all the atoms in the model as nodes and all the supported rules as nodes as welll
    \item $V = I \cup \{ \lbl{r} \mid r \in P \land I \models \Bd{r} \}$
    % take all outgoing edges from atoms such that the corresponding (neg/pos) literal is supported by the model
    \item $\forall (a, \ell) \in \Epos$ such that $a \in I$, then $(a, \ell) \in E$
    % \item $\forall (a, \ell) \in \Eneg$ such that $a \notin I$, then $(a, \ell) \in Em_{-}$   % we disregard the negative edges
    % take all outgoing edges from rules if the rule is supported by the model
    \item $\forall (\ell, a) \in \Epos$ such that $I \models a \wedge \Bd{\InvLb{\ell}}$, then $(\ell, a) \in E$
  \end{itemize}
\end{definition}
...
\begin{proposition}
  ...
  the \emph{Model Subgraph} $M\pg{P}^I$ is a subgraph of the \emph{Program Graph} $\pg{P}$.
\end{proposition}
%
\begin{figure}
  \centering
  ...
  The entire program graph $\pg{P_{1}} = \langle \Vc \cup \Vd \cup \Va, \Epos\cup \Eneg \rangle$ can be seen, but the model subgraph $\mpg{P_1}^{M1}$, which is highlighted  the answer set $M1 = \{w,a,d\}$.}
...
\begin{itemize}
  ...
  \item That is
  \begin{itemize}
    \item The program graph of the union of two programs $\pg{\pr \cup \pf} = \langle \Vc \cup \Vd \cup \Va, \Epos, \Eneg \rangle$ where $\pr$ and $\pf$ are the \emph{reference} and \emph{foil} programs, respectively.
    \item A \emph{reference model subgraph} $\mpg{\pr}^{I_r} = \langle V_r, E_r \rangle$, for some $I_r$ such that $I_r \models \pr$
    \item The \emph{foil model subgraph} $\mpg{\pf}^{I_f} = \langle V_f, E_f \rangle$, for some $I_f$ such that $I_f \models \pf$
...
\begin{definition}[Contrastive Subgraph]
  ...
  \begin{itemize}
    ...
    \item
    two programs $\pr$ and $\pf$,
    \item
    a program graph $\pg{\pr \cup \pf} = \langle \Vc, \Vd, \Va, \Epos, \Eneg \rangle$,
    ...
    \item
    a program subgraph $\mpg{\pr}^{I_r} = \langle V_r, E_r \rangle$
    \item
    and
    a program subgraph $\mpg{\pf}^{I_f} = \langle V_f, E_f \rangle$ such that $I_f \models Q$
  \end{itemize}
  then
  %
  ...
  CM_{\pr, \pf}^{I_r, I_f} =
  \langle
    \Vc, % 1. reference nodes, foil nodes, constraint nodes (or conflict nodes?)
    \langle E_c, \Eneg \rangle  % fact/foil positive edges, negative edges
  \rangle
  \]
  %
  where
  \begin{itemize}
    \item  $\Vc = V_r \cup V_f \cup \{ \lbl{r} \mid (r \in \pr \cup \pf) \land (\iscons{r}) \land (I_r, I_f \models \Bd{r}) \}$
    is the set of constraint nodes that are existentially satisfied by both reference and foil models

    \item $E_c = E_r \cup E_f$
    \item $E_{c-} = \{ (a, \ell) \mid (a, \ell) \in \Eneg \land [ (I_r \models a) \oplus (I_f \models a)]\}$
...
%
\begin{proposition}
  Given a \emph{contrastive graph} $CM_{\pr, \pf}^{I_r, I_f}$,
  \begin{itemize}
    \item the \emph{model graph} $\mpg{\pr}^{I_r}$ is a subgraph of $CM_{\pr, \pf}^{I_r, I_f}$
    \item the \emph{model graph} $\mpg{\pf}^{I_f}$ is a subgraph of $CM_{\pr, \pf}^{I_r, I_f}$
    \item $CM_{\pr, \pf}^{I_r, I_f}$ is a subgraph of the \emph{program grpah} $\pg{\pr \cup \pf}$.
  \end{itemize}
\end{proposition}
...
\begin{itemize}
  \item We may separate contrasts in types depending on the (dis)equalities among the programs $\pr$ and $\pf$, and the masks $MG_r$ and $MG_f$.
  \item We propose the following types
  \begin{itemize}
    \item when $\pr = \pf \land I_r = I_f$ then we call it \emph{positive justification}.
    \item when $\pr = \pf \land I_r \neq I_f$ then we call it \emph{alternative contrastive explanation}.
...
\begin{itemize}
  %
  \item We start from a \emph{reference} program $\pr$ and a particular answer set $I_r$ of $\pr$.
  %
  \item Query $Q$ is a set of literals.
  %
  \item The process of \emph{abduction} is the process of finding a \emph{foil program} $\pf$ and an answer set $I_f$ of $\pf$ such that $I_f \models Q$.
  %
  \item The way we obtain $\pf$ and $I_f$ is by adding and removing rules from $\pr$ in a controlled way.
  %
\end{itemize}
%
...
\begin{definition}{Abducible programs}
  ...
  \begin{itemize}
    ...
    %
  \end{itemize}
  we denote $AP(\pr, \addable, \removable)$ as the set of all programs $\pf$ that satisfy
  \[
    \pr \setminus \removable \subseteq \pf \subseteq (\pr \cup \addable)
  \]
  Additionally, we say that $\pf$ is a valid \emph{abducible program from $\pr$ with respect to $\addable$ and $\removable$} if $\pf \in AP(\pr, \addable, \removable)$.
\end{definition}
%
...
\begin{definition}{Abducible Worlds}
  ...
  \begin{itemize}
    ...
    %
  \end{itemize}
  we denote $AW(\pr, \addable, \removable)$ as the set of all pairs $(\pf, I_f)$ satisfying
  \begin{itemize}
    \item $\pf \in AP(\pr, \addable, \removable)$
    \item $I_f \models \pf$
  \end{itemize}
  we call any of the pairs in the set as an \emph{abducible world from $\pr$}
\end{definition}
%
\begin{itemize}
  \item Note that we allow $\pr = \pf$
\end{itemize}
%
...
\begin{definition}[Valid Foil World]
  ...
  \begin{itemize}
    ...
    %
    \item a set of \emph{removable} labelled rules $\removable \subseteq \pr$ of the form \eqref{f:disjunctive-rule} and \eqref{f:choice-rule},
    \comment{S: I suggest using $R_{\mathit{add}}$ and $R_{\mathit{rem}}$ or $R_{+}$ and $R_{-}$.}
    %
    \item and
    %
    \item a set of \emph{addable} labelled rules $\addable$ of the form \eqref{f:disjunctive-rule} and \eqref{f:choice-rule},
  ...
  we say the pair
  \[(\pf, I_f)\],
  is a \emph{valid foil world for $\pr$ with respect to $Q$} iff:
  \begin{itemize}
    \item $(\pf, I_f) \in AW(\pr, \addable, \removable)$
    \item $I_f \models Q$
  \end{itemize}
\end{definition}
%
...
\begin{itemize}
  %
  \item Regarding contrastive explanations, from a \emph{reference world} and a query $Q$, the idea is to find a valid foil world $(\pf, I_f)$ with with respect to $Q$, that will be later used to compute a \emph{model subgraph} $\mpg{\pf}^{I_f}$, and then create a contrast graph $\cpg{\pr, \pf}^{I_r, I_f}$.
  %
  \item {\color{blue} This definition does not impose the existence of a \emph{reference model} $I_r \models \pr$ neither that $I_r \nvDash Q$.} It's convenient as allow particular scenarios:
  \begin{itemize}
    \item \emph{unsat reference programs} $M(\pr) = \emptyset$ and $Q = \emptyset$, that is, we only want to recover satisfiability; or (b), $Q \neq \emptyset$, we want it to be sat and force something.
  ...
  %
  \item The sets $\removable$ and $\addable$ is what we informally call the \emph{abducibles}, that is, what we can add or remove to the original program in order to satisfy the query.
  %
  ...
  \begin{itemize}
    %
    \item $A_f = \pf \setminus \pr \subseteq \addable$, that is, the set of rules that were \emph{added}.
    %
    \item $R_f = \pr \setminus \pf \subseteq \removable$, that is, the set of rules that were \emph{removed}.
    %
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    %
    \item If $I_r \nvDash Q$, then $Q$ is a \emph{Why not query}, asking for contrastive explanation of $Q$ starting from $\pr$ and $I_r$ (and some sets $\addable$ and $\removable$).
  \end{itemize}
  %
...
\begin{definition}{Closeness Function}
  ...
  \[
  cf : AW(\pr, \addable, \removable) \times AW(\pr, \addable, \removable) \to \mathbb{Z}
  \]
...
\begin{itemize}
  ...
  \item {\color{red} ISSUE: This closeness function requires to provide a reference program-model pair, so it does not work when $\pr$ is unsat. Need to fix this.}
\end{itemize}
...
\begin{definition}[Closest Foil World]
  ...
  \begin{itemize}
    ...
    \item a set of \emph{addable} labelled rules $\addable$ of the form \eqref{f:disjunctive-rule} and \eqref{f:choice-rule},
    %
    \item Let $Foils(\pr, \addable, \removable, Q)$ be the set of all valid foil worlds $(\pf, I_f) \in AW(\pr, \addable, \removable)$ for $\pr$ with respect to $Q$.
    %
    and
    %
  ...
  \end{itemize}
  then
  the \emph{closest foil world for $\pr$ with respect to $Q$ and $cf$} is given by

  \[
  \mathrm{CFW}(\pr, \addable, \removable, Q;cf)
  := \argmin_{(\pf, I_f) \in Foils(\pr, \addable, \removable, Q)} \; cf(I_r, I_f).
  \]
...
%   and an answer set $I \subseteq \Va$ of $P$,
%   an \emph{mask} of $\pg{P}$ under $\addable$ is a function $\lambda: I \rightarrow \Vc \cup \Vd$ such that:
%   \begin{itemize}
...
% %   Given a program graph $\pg{P} = \langle \Vc \cup \Vd \cup \Va, \Epos, \Eneg \rangle$,
% %   and an answer set $I \subseteq \Va$ of $P$,
% %   an \emph{mask} of $\pg{P}$ under $\addable$ is a function $\lambda: I \rightarrow \Epos$ such that:
% %   \begin{itemize}
% %     % Lambda maps each atom to one of its incoming edges
% %     \item $\forall a \in I, \lambda(a) = (l, a) \land l \in \Epos$
...
%   Given a program graph $\pg{P} = \langle \Vc \cup \Vd \cup \Va, \Epos, \Eneg \rangle$,
%   and an answer set $I \subseteq \Va$ of $P$,
%   a \emph{mask} of $\pg{P}$ explaining $I$ is a set $EM \subseteq \Epos$ such that:
%   \begin{itemize}
...
%     \item $forall (l, a) \in EM$, such that $I \models \InvLb{l}$, then $\forall (a', l) in \Epos$ it must hold that $(a', l) \in EM$.
...
%   \item For any answer set $I$ of a program $P$ there is always at least one mask $\lambda$ such that the technical explanation $\pg{\lambda}$ is a directed acyclic graph (DAG) (pontentially many). \comment{I have to prove this by proving the correspondance with Justified Models.}
...
%   corresponding to a ASP program $P$,
%   and an answer set $I \subseteq \Va$ of $P$,
%   and a \emph{edge mask} $EM \subseteq \Epos$ explaining $I$,
%   an explanation of $I$ under $P$ is a directed graph
%    \[    \pg{EM} = \langle V, EM \rangle    \]
%   where
...
%   \caption{Mask.
%   The entire program graph $\pg{P_{1}} = \{\Vc, \Vd, \Va, \Epos, \Eneg\}$ can be seen, but the highlighted part of the graph explains the answer set $M_1 = \{s,w,a,d\}$.
%   This subgraph is the techincal explanation $\pg{\lambda} = \langle V, E \rangle$ induced from a mask $\lambda: M_1: \rightarrow $ where $\lambda(s) = 1$, $\lambda(w) = 4$, $\lambda(a) = 2$ and $\lambda(a) = 3$.}
%   \label{fig:mask}
% \end{figure}
% %
...
%   corresponding to a ASP program $P$,
%   and an answer set $I \subseteq \Va$ of $P$,
%   and an injective function $\lambda: I \rightarrow \Vc \cup \Vd$ such that selecting a rule $r$ such that $I \models r$ to justify each atom,
...
% % %
% % \begin{proposition}
% %     Let $\pg{P} = \{\Vc, \Vd, \Va, \Epos, \Eneg\}$ be the program graph for program $P$,
% %     and a mask \lambda $\pg{\lambda} = \langle V_t, E_t \rangle$ be a causal technical explanation of an answer set $I$ under a program $P$,
% %     then graph $G^s = forget(\pg{\lambda}, \Vc \cup \Vd)$ is a valid support graph of $I$ under $P$.
% % \end{proposition}
% % %
...
%     let
%     $\pg{ref} = \langle I_{ref}, E_{ref}, \lambda_{ref} \rangle$ be a support graph explanation of $I_{ref}$ with respect to $P_{ref}$,
%     and
%     $\pg{alt} = \langle I_{alt}, E_{alt}, \lambda_{alt} \rangle$ be a support graph explanation of $I_{alt}$ with respect to $P_{alt}$.
...
% %     $I_{ref} \in CM(P_{ref})$ and $I_{alt} \in CM(P_{alt})$ be two classical models of $P_{ref}$ and $P_{alt}$ respectively,
% %     let
% %     $\pg{ref} = \tuple{I_{ref}, E_{ref}, \lambda_{ref}}$ be a support graph explanation of $I_{ref}$ with respect to $P_{ref}$,
% %     and
% %     $\pg{alt} = \tuple{I_{alt}, E_{alt}, \lambda_{alt}}$ be a support graph explanation of $I_{alt}$ with respect to $P_{alt}$.
