...
\begin{example}[Program Graph]\label{ex:example1}
  ...
  %%
  \lstinputlisting[caption={Modified James Bond Example. Program $P_1$}, language=clingos, label={lst:example1}]{encodings/example1.lp}
  %%
...
\paragraph{Discussion on having or not negative literals in the model subgraph}
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      S2B: Wouldn't we get this when we do the contrast?}
...
\begin{itemize}
  ...
  %
  \item \textbf{Visualization:}
  \comment{S: This is how we visualize the contast, the valid foil is just the foil model subgraph}
  \begin{itemize}
...
\begin{example}[Selecting minimal foils using the program-difference cost]\label{ex:best-foils}
  ...
  \mathrm{BestFoils}(\pr, \addable, \removable, Q, \pdiff)
    = \{(\pf, \m_f) \in \foils(\pr, \addable, \removable, Q)
        \mid |\,\pr \triangle \pf\,| = 1\}.
  \]
  %
