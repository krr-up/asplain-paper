...
\begin{example}[Program Graph]\label{ex:example1}
  ...
  \comment{TODO: match labels with $P1$, also remove the natural language, also remove optional input}
  %
  \begin{figure}
    \centering
  ...
  %
  Rectangle-nodes in graph $G_{P_{1}}$ represent the body of some rule in $P1$, in particular, sets $\Vc = \{1\}$
  and $\Vd = \{2,3,4,5,6\}$ correspond to the set of rules in $P1$ whose head is a choice, respectively a disjunction.
  %
...
\begin{itemize}
  \item
  $\Vc$, $\Vd$, $\Va$ are pairwise disjoint \comment{S: This would be simpler to show (I think) if the sets for \isdisj{r} is rather Dis(P), for now I am trying to be consistent and if it an upper case like Atoms or Foils then is a set}
  \item
...
\begin{itemize}
  ...
  % \end{itemize}
  \item Notice, it {\color{blue} does not select any particular derivation} when alternative causes emerge (that is, is not causal) (is not equivalent to \cite{cabmun24a} but could be equivalent to \cite{altrsoba23a})
\end{itemize}
...
\begin{itemize}
  ...
  \comment{S: Do we have to? or can we just say that these can be maped back into a single cost function? @Son perhaps you can help with this?}
\end{itemize}
...
\begin{itemize}
  ...
  \item Nodes and eges that are not in $\mgpi{\pf}{\m_f}$, is shown with less opacity.
  \item Rule nodes that can be removed (i.e., that are in $\removable$) are shown with a dashed border.
  \item The atom nodes, for any atom used in the query $Q$ are shown with a thicker border. \comment{Not in the image yet, need to update}
  \item Removed rules \removed are shown with a red border, whereas added rules \added are shown with a blue border.
