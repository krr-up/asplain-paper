...
\begin{example}[Abduction for UNSAT]
  %
...
\begin{definition}[Contrastive Subgraph]
  ...
  \begin{itemize}
    \item a set of literals $Q$ refered as \emph{query},
    %
    \item a reference program $P_r$ and a particular program subgraph $MG_{P_r}^{I_r} = \langle V_r, E_r \rangle$ such that $I_r \not\models Q$,
    %
    \item sets $A$ and $R$, of \emph{addable} (respectively \emph{removable}) rules, and a \emph{valid foil model subgraph} $MG_{P_f}^{I_f} = \langle V_f, E_f \rangle \in Foils(P_r, A, R, Q)$
    %
  ...
  CM_{P_r, P_f}^{I_r, I_f} =
  \langle
    V, % 1. reference nodes, foil nodes, constraint nodes (or conflict nodes?)
    \langle E, E_{-} \rangle  % fact/foil positive edges, negative edges
  \rangle
  \]
  %
  where
  \begin{itemize}
    \item  $V = V_r \cup V_f \cup  \{ \Lb{r} \mid (r \in P_r \cup P_f) \land (Cons(r)) \land (I_r, I_f \models \Bd{r}) \}$
    \comment{
      {\bf Son:} what about other rules whose body is existentially satisfied by $I_r$ and $I_f$, but not by $I_r$ nor $I_f$ alone?
    }

    \item $E = E_r \cup E_f$
    \item $E{-} = \{ (a, \ell) \mid (a, \ell) \in E_{-} \land [ (I_r \models a) \oplus (I_f \models a)]\}$
  \end{itemize}
\end{definition}
%
\begin{proposition}
...
\begin{example}[Contrastive Explanations for a query]
  ...
  %
  For this particular example, there are two possible contrastive explanations: 
  one consisting on picking $w$ (working day) instead of $h$, which leads to James taking the antidote (Figure~\ref{fig:example});
  and another consiting on removing $d$ (drinking the poison) (Figure~\ref{fig:example}).
  %
  The latter is an \emph{alternative contrastive explanation}, the former is a \emph{hypothetical contrastive explanation}
  % 
\end{example}

\begin{example}[Positive Justification as an special case of Contrastive Explanation]
...
%%%%%%%%%%%%%%% EXPLANATION PREFERENCE (ABDUCTION)
