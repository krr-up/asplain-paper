...
\begin{itemize}
  ...
  \begin{itemize}
    \item We show the model subgraph coloured over the original program graph, like in Figure~\ref{fig:model-subgraph}.
...
\begin{itemize}
  %
  \item We start from a \emph{reference} program $P_r$.
  %
  \item Query $Q$ is a set of literals.
  %
  \item \emph{Abduction} is the process of finding a \emph{foil program} $P_f$ and an answer set $I_f$ of $P_f$ such that $I_f \models Q$.
  %
...
\begin{definition}[Valid Foil]\label{def:valid-foil}
  ...
  \end{itemize}
  %
  % we say the model subgraph $MG_{P_f}^{I_f}$
  we say the pair $(P_f, I_f)$
  is a \emph{valid foil for $P_r$ with respect to some $A$, $R$ and $Q$} iff:
  \begin{itemize}
    \item $ P_r \setminus R \subseteq P_f \subseteq (P_r \cup A)$
    \item $I_f \in AS(P_f)\ \land\ I_f \models Q$
  \end{itemize}
  Additionally, we denote $Foils(P_r, A, R, Q)$ as the set of \emph{all valid foils for $P_r$ with respect to $A$, $R$, and $Q$}.
\end{definition}
%
\paragraph{Discussion on Valid Foil (Def. \ref{def:valid-foil})}
\begin{itemize}
  \item Intuitively, for obtaining a valid foil you can take $P_r$, and you may add/remove anything from $A$ and $R$ to obtain a new foil program $P_f$. For any model $I_f$ of $P_f$ that satisfies the query $Q$, you can form a valid foil.
  %
  \item Also, note that $P_f$ may be equal to $P_r$ meaning that we did not add or remove anything from $P_r$ and we still find a $I_f$ model that satisfies the query. That is, there was another model of $P_r$ that satisfy the query and can be used to explain \emph{why not} $Q$.
  %
  \item In the case that the set $Foils(P_r, A, R, Q) = \emptyset$, this means that there is no way to satisfy the query even if removing or adding any combination from $R$ or $A$.
  %
  \item Of course, note that there is no restrictions over both the $R$ or $A$ sets, which means that one could put potentially anything there. Therefore, some $A$ or $R$ sets can be weird.
  \begin{itemize}
    \item For instance, adding rules which do not interact at all with the rest of rules in $P_r$ (they do not share language).
  \end{itemize}
  %
  % \item Regarding contrastive explanations, from a \emph{reference program} and a query $Q$, the idea is to find a \emph{foil valid subgraph} $MG_{P_f}^{I_f}$ with with respect to $Q$, that will be later used to compute a contrast graph $CG_{P_r, P_f}^{I_r, I_f}$.
  %
  \item {\color{blue} This definition does not impose the existence of a \emph{reference model} $I_r \models P_r$ neither that $I_r \nvDash Q$.} It's convenient for allowing particular scenarios:
  \begin{itemize}
    \item \emph{unsatisfiable reference programs}: $AS(P_r) = \emptyset$ and $Q = \emptyset$, that is, we only want to recover satisfiability; or (b), $Q \neq \emptyset$, we want it to be sat and force something.
    \item The reference and foil models may collide $I_r = I_f$, which means that we are in fact doing no contrast. In other words, $I_r$ already satisfy the query.
  \end{itemize}
  %
  \item The sets $R$ and $A$ is what we informally call them \emph{abducibles}, that is, rules that we can add or remove to the original program in order to satisfy the query.
  %
  ...
  \begin{itemize}
    ...
    %
    \item The role of $A_f$ and $R_f$ will play an important role when designing and implementing good explanation preferences.
  \end{itemize}
  %
  \item \textbf{Visualization:}
  \begin{itemize}
    \item We now have to handle the removals and the additions, in other words, we have to deal $P_r \neq P_f$.
    \item To do so, we use the program graph $PG_{P_r \cup Pf}$ as the base of the visualization.
    \item We highlight $MG_{P_f}^{I_f}$ as an explanation of the foil.
    \item Finally we also highlight the added and removed rules using the sets $A_f$ and $R_f$.
    \item Examples: Figures~\ref{fig:example2}, \ref{fig:example3-1} and~\ref{fig:example3-2}.
  \end{itemize} 
\end{itemize}
%

\begin{example}[Removing rules for explaining a query]\label{ex:example2}
  %
  ...
  %
  Now, imagine that a user is shown $P$\ref{lst:example2}'s unique model $M = \{ h, d, p \}$, and let's say that she is surprised that James is poisoned and wants to know why.
  %

  %
  To answer such a question, we can use the abuction process discussed in Definition~\ref{def:valid-foil}.
  %
  We first need to define the users query $Q = \{ \neg p \}$, which matches her expectations.
  %
  Also, we will consider the actions performed by James as \emph{removables} so that we can reason about what would have happened if James had acted differently.
  %
  To that aim, let us consider rule $4$ (James have drank) as a \emph{removable rule} by setting $R = \{ 4 \}$.
  %
  In this case, we will not have any \emph{addable rule}, that is $A = \emptyset$.
  %
  To generate an explanation, we would take any valid foil $(P_f, I_f) \in Foils(P2, A, R, Q)$ and compute the foil model subgraph $MG_{P_f}^{I_f}$.
  %
  For a nicer visualization, this model subgraph can be depicted over the program graph $PG_{P_r \cup P_f}$ as in Figure~\ref{fig:example2}.
  %
\end{example}
%
\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{resources/example2.png}
  \caption{Visualization of the valid foil of Example~\ref{ex:example2}}
  \label{fig:example2}
\end{figure}
...
\begin{example}[Abduction for UNSAT]\label{ex:example3}
  ...
  %
  First we will consider $P$\ref{lst:example3}$ = P_r$
  and
  $Q = \emptyset$ since we are only interested in recovering satisfiability.
  Also, we will always consider $A = \emptyset$ for this example.
  %

  %
  If we say that $R = \{ 4 \}$, then there is only one \emph{valid foil subgraph} depicted in Figure~\ref{fig:example3-1}.
  %
  If we however consider $R = \{ 2, 3 \}$, then there are three possible \emph{valid foils} depicted in Figure~\ref{fig:example3-2}.
\end{example}
%
\begin{figure}
  \centering
  \includegraphics[width=0.3\textwidth]{resources/example3-1.png}
  \caption{Valid foil for $P$\ref{lst:example3} with respect to $Q = \emptyset$, when $R = \{ 4 \}$. Only (constraint) rule 4 was removed.}
...
\begin{itemize}
  ...
  %
  \item Since our framework allows full flexibility in generating possible \emph{Foil Worlds}, we also need a good way to identify the most appropriate ones.
  %
  \item
  ...
  \begin{itemize}
    \item the number of rules that must be added or removed,
    % \item the differences between what holds true or false in the two models,  % this is for contrastive
    \item the preference for modifying interactively added rules first, and
...
\begin{definition}[Foil cost function]\label{def:foil-cost}
  ...
  \begin{itemize}
    \item a query $Q$,
    %
    \item a reference program $P_r$,
    %
    \item a set of removable labelled rules $R \subseteq P_r$,
    %
    \item and a set of addable labelled rules $A$,
  \end{itemize}
  %
  a \emph{foil cost function} for $(P_r, A, R, Q)$ is a function
  %
  \[
    \mathit{foil\_cost}_{P_r,A,R,Q} :
    \mathrm{Foils}(P_r, A, R, Q) \rightarrow \mathbb{N}.
  \]
  that assigns each foil $(P_f, I_f)$ a numerical value representing a cost with respect to the reference program.
\end{definition}
...
\begin{definition}[Best foils]\label{def:best-foils}
  ...
  \begin{itemize}
    ...
    \item and a set of addable labelled rules $A$,
    %
    \item and a foil cost function $\mathit{foil\_cost}_{P_r,A,R,Q}$,
  \end{itemize}
  %
  the set of \emph{best foils} is defined as
  %
  \[
  \mathrm{BestFoils}(P_r, A, R, Q, \mathit{foil\_cost})
    = \operatorname*{arg\,min}_{(P_f, I_f) \in \mathrm{Foils}(P_r, A, R, Q)}
      \mathit{foil\_cost}_{P_r,A,R,Q}(P_f, I_f).
  \]
\end{definition}
...
\begin{itemize}
  ...
  %
  \item Then, $\mathrm{BestFoils}$ contains all valid foils that achieve the minimal value of the selected cost function.
  %
...
\begin{example}[Program-difference cost function]\label{ex:program-difference-cost-function}
  For a given abduction setup $(P_r, A, R, Q)$.
  %
  We define the particular foil cost function \emph{program difference},
  % \[
  %   \mathit{PDiff}_{P_r,A,R,Q} :
  %   \mathrm{Foils}(P_r,A,R,Q) \rightarrow^{} \mathbb{N}
  % \]
  defined as follows:
  \[
    \mathit{PDiff}_{P_r,A,R,Q}(P_f, I_f)
    = |\,P_r \,\triangle\, P_f\,|,
  \]
...
\begin{itemize}
  ...
  %
  \item Minimizing $\mathit{PDiff}$ therefore selects foils that are syntactically closest to the reference program.
  %
  ...
  \[
  \mathit{PDiff}_{P_r,A,R,Q}(P_f, I_f)
    = |R'| + |A'| = |P_r \setminus P_f| + |P_f \setminus P_r|
  \]
\end{itemize}

\begin{example}[Selecting minimal foils using the program-difference cost]\label{ex:best-foils}
  Recall Example~\ref{ex:example3} and the three valid foils depicted in Figure~\ref{fig:example3-2}.
  %
  Among these, the rightmost foil is clearly redundant with respect to the other two, as it removes both rules~2 and~3, whereas the left and middle foils remove only one each.
  %
  By applying the program-difference cost function
  $\mathit{PDiff}$
  we assign each foil a value proportional to the number of rules that differ from the reference program.
  %
  In this case, foils that remove a single rule receive a cost of $1$, while the foil removing both rules receives a cost of $2$.
  %
  Therefore,
  \[
  \mathrm{BestFoils}(P_r, A, R, Q, \mathit{PDiff})
    = \{(P_f, I_f) \in \mathrm{Foils}(P_r, A, R, Q)
        \mid |\,P_r \triangle P_f\,| = 1\}.
  \]
  %
  Minimizing $\mathit{PDiff}$ thus selects the left and middle foils from Figure~\ref{fig:example3-2}, which represent the \emph{cardinality}-minimal deviations from the reference program needed to recover satisfiability.
\end{example}

\paragraph{Discussion on Example~\ref{ex:best-foils}}
\begin{itemize}
  \item This example illustrates the use of the preferences to select the foils which made the least changes to the original program.
\end{itemize}

\paragraph{Open Questions/ Future}
\begin{itemize}
  \item {\color{red} Can we write a subset minimal preference on the additions/removals? (related to UNSAT CORE) Might we use asprin for this?}
  \item Do we need to provide an example of a domain-driven criteria? Where removing/adding certain rules weight more than others?
\end{itemize}
...
\begin{itemize}
  ...
  %
  \item We will be able to use causal relations \emph{hidden} in constraints by using \emph{existential satisfaction}, defined below.
  %
\end{itemize}
...
\begin{definition}[Contrastive Subgraph]
  ...
  \begin{itemize}
    \item a set of literals $Q$ refered as \emph{query},
    %
    \item a reference program-model pair $\langle P_r, I_r \rangle$ such that $I_r \models P_r$, $I_r \not\models Q$, and its correspondant subgraph $MG_{P_r}^{I_r} = \langle V_r, E_r \rangle$,
    %
    \item sets $A$ and $R$, of \emph{addable} (respectively \emph{removable}) rules, a \emph{valid foil program-model pair} $(P_f, I_f) \in Foils(P_r, A, R, Q)$, and its correspondant model subgraph $MG_{P_f}^{I_f} = \langle V_f, E_f \rangle$.
    %
    \item a program graph $G_{P_r \cup P_f} = \langle V_c, V_d, V_a, E_{+}, E_{-} \rangle$,
    \comment{
  ...
  \langle
    ...
    \langle Econ, Econ_{-} \rangle  % fact/foil positive edges, negative edges
  \rangle
  \]
  %
  where
  \begin{itemize}
    \item  $Vcon = V_r \cup V_f \cup  \{ \Lb{r} \mid (r \in P_r \cup P_f) \land (Cons(r)) \land (I_r, I_f \models \Bd{r}) \}$
    \comment{
      {\bf Son:} what about other rules whose body is existentially satisfied by $I_r$ and $I_f$, but not by $I_r$ nor $I_f$ alone?
    }

    \item $Econ_{+} = \{(v1, v2) \mid (v1, v2) \in E_{+} \land v1,v2 \in Vcon\}$
    \item $Econ_{-} = \{(v1, v2) \mid (v1, v2) \in E_{-} \land v1,v2 \in Vcon\}$
    \item $Econ_{-} = \{ (a, \ell) \mid (a, \ell) \in E_{-} \land [ (I_r \models a) \oplus (I_f \models a)]\}$
  \end{itemize}
\end{definition}
%
\begin{proposition}
...
\begin{itemize}
  \item We may separate contrasts in types depending on the (dis)equalities among the model subgraphs used $MG_{P_r}^{I_r} = \langle V_r, E_r \rangle$ and $MG_{P_f}^{I_f} = \langle V_f, E_f \rangle$.
  \item We propose the following types
  \begin{itemize}
    \item when $P_r = P_f \land I_r = I_f$, so that $I_r \models Q$ then we call it \emph{positive justification}.
  ...
  \end{itemize}
  \item Note that, when we start from an unsat program (as in Section~\ref{sec:abductive-explanations}) we can only compute hypothetical contrastive explanations as we need a foil program $P_f$ for having any $I_f \models Q$.
\end{itemize}

\paragraph{Finding Contrastive Explanation for Queries}
\begin{itemize}
  \item We first start from the assumption that there exists some referece model $I_r$ of some reference program $P_r$ that has been shown to (or selected by) the user.
  %
  \item Then the user wonders why some query $Q$ is not true in the reference.
  %
  \item We use abduction to find a $MG_{P_f}^{I_f} \in Foil(P_r, A, R, Q)$, for some $R$ and $A$.
  %
  \item Sets $R$ and $A$ can vary depending on the particular application.
  %
  \item Finally, we compute $CM_{P_r, P_f}^{I_r, I_f}$
\end{itemize}


\begin{example}[Contrastive Explanations for a query]\label{ex:example4}
  Please recall Program $P$\ref{lst:example1}
  %
  Imagine that the user is shown with a visualization of $M2 = \{h, d, p\}$ and wonders why James has been poisoned, that is, user expected James not to be poisoned.
  %
  Now, if we set $Q = \{ \neg p \}$,
  $A = \emptyset$
  and
  $R = \{ 4 \}$ (allowing to remove the fact $d$),
  %
  then for any $MG_{P_f}^{I_f} \in Foil(P_r, A, R, Q)$,
  %
  we can build a contrastive graph $CM_{P_r, P_f}^{I_r, I_f}$.
  %
  
  %
  For this particular example, there are three possible contrastive explanations, depicted in Figure~\ref{fig:example4} from left to right respectively: 
  (1) one consisting on picking $w$ (working day) instead of $h$, which leads to James taking the antidote;
  (2) another consiting onletting $h$ still true, but removing $d$ (drinking the poison);
  and finally, (3), we can do both picking $w$ and removing $d$, which would still lead to $\neg p$.
  %
  The first is an \emph{alternative contrastive explanation} as the program has not been changed (that is, $P_r = P_f$), the other two are \emph{hypothetical contrastive explanations}
  %
\end{example}
%
\begin{figure}
  ...
  \begin{minipage}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{resources/example4-1.png}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{resources/example4-2.png}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{resources/example4-3.png}
  \end{minipage}
  \caption{Possible contrastive explanations for the pair $(P1, M2)$ and $Q = \{ \neg p \}$, when $A = \emptyset$ and $R = \{ 4 \}$}
  \label{fig:example4}
\end{figure}
...
\begin{itemize}
  \item The reference model subgraph $MG_{P_r}^{I_r}$ is coloured in purple whereas the foil model subgraph $MG_{P_f}^{I_f}$ is coloured in green.
  \item Also, what is not part of $MG_{P_f}^{I_f}$, is shown with less opacity.
  \item The atom nodes, for any atom used in the query $Q$ are shown with a thicker border.
  \item As when visualizing foils (for instance, in Figures~\ref{fig:example3-1} and~\ref{fig:example3-2}), removals and additions (e.g. the $R' = P_r \setminus P_f$ and the $A' = P_f \setminus P_r$ sets) are shown in red and blue respectively.
  \item Finally, for those constraint rule nudes, that are existentially statisfied by $I_r$ and $I_f$ together, we colour them with the orange color.
...
%
\paragraph{Reading of Contrastive Explanations}
\begin{itemize}
  \item Figure~\ref{fig:example4}, left:
  \begin{itemize}
    \item James was poisoned ($p$) because he did not take the antidote ($\neg a$) because it was a holiday ($h$).
    \item If it had been a weekday ($w$), James would have taken the antidote ($a$) and therefore he would not have been poisoned ($\neg p$).
  \end{itemize}

  \item Figure~\ref{fig:example4}, center:
  \begin{itemize}
    \item James was poisoned ($p$) because he drank the poison ($d$).
    \item If James had not drunk the poison ($\neg d$), he would not have been poisoned ($p$).
  \end{itemize}

  \item Figure~\ref{fig:example4}, right:
  \begin{itemize}
    \item James was poisoned ($p$) because he drank the poison ($d$), and since it was a holiday ($h$), he did not have the antidote ($\neg a$).
    \item If James had not drunk the poison ($\neg d$), or if it had not been a holiday ($h$), James would not have been poisoned ($p$).
  \end{itemize}

  \item Although the explanation in the right contains more information, one could claim that it is redundant with the other two, as the other two need fewer changes and equally explain the query.
  \item We will use the preferences to select among them
\end{itemize}
%
...
\begin{itemize}
  \item In practice, we can still use the $\mathit{foil\_cost}$ plus $\mathit{BestFoils}$ approach to select among the best foil to make a contrast with.
  %
  ...
  %
  \item For instance, in Example~\ref{ex:example4}, we cannot notice any difference between contrastive explanations (2) and (3) by using the $\mathit{PDiff}$ foil cost function alone, since they are equivalent in terms of removed rules.
  %
  \item However, explanation (2) requires less changes, as in explanation (3) we moreover had to pick $w$ instead of $h$.
  %
  \item In the following, we define an extension of the previous definitions~\ref{def:foil-cost} and~\ref{def:best-foils}, for defining preference that include a \emph{reference} model $I_R$.
\end{itemize}
...
\begin{definition}[Contrast cost function]\label{def:contrast-cost}
  ...
  \begin{itemize}
    \item a query $Q$,
    %
    \item a reference program-model pair $(P_r, I_r)$,
    %
    \item a set of removable labelled rules $R \subseteq P_r$,
    %
    \item and a set of addable labelled rules $A$,
  \end{itemize}
  %
  a \emph{contrast cost function} for $((P_r, I_r), A, R, Q)$ is a function
  %
  \[
    \mathit{contrast\_cost}_{(P_r, I_r),A,R,Q} :
    \mathrm{Foils}(P_r, A, R, Q) \rightarrow \mathbb{N}.
  \]
  that maps each foil $(P_f, I_f)$ to a numerical value representing its cost with respect to the reference program-model pair $(P_r, I_r)$.
\end{definition}
...
\begin{definition}[Best constrast]\label{def:best-contrasts}
  ...
  \begin{itemize}
    \item a query $Q$,
    %
    \item a reference program $(P_r, I_r)$,
    %
    \item a set of removable labelled rules $R \subseteq P_r$,
    %
    \item and a set of addable labelled rules $A$,
    %
    \item and a contrast cost function $\mathit{contrast\_cost}_{(P_r, I_r),A,R,Q}$,
  \end{itemize}
  %
  the set of \emph{best contrasts} is defined as
  %
  \[
  \mathrm{BestContrasts}((P_r, I_r), A, R, Q, \mathit{contrast\_cost})
    = \operatorname*{arg\,min}_{(P_f, I_f) \in \mathrm{Foils}(P_r, A, R, Q)}
      \mathit{contrast\_cost}_{(P_r, I_r),A,R,Q}(P_f, I_f).
  \]
\end{definition}

\begin{example}[Model-difference cost function]\label{ex:model-difference-cost-function}
  For a given abduction setup $(P_r, A, R, Q)$ with reference model $I_r$,
  we define the \emph{model-difference} contrast cost function as
  \[
    \mathit{MDiff}_{(P_r, I_r),A,R,Q}(P_f, I_f)
      = |\,I_r \,\triangle\, I_f\,|,
  \]
  where $\triangle$ denotes the symmetric difference between the models $I_r$ and $I_f$.
...
\begin{example}[Reference–Foil difference cost function]\label{ex:reference-foil-difference-cost-function}
  ...
  \[
    \mathit{RFDiff}_{(P_r, I_r),A,R,Q}(P_f, I_f)
      = \mathit{MDiff}_{(P_r, I_r),A,R,Q}(P_f, I_f)
      + \mathit{PDiff}_{P_r,A,R,Q}(P_f, I_f).
  \]
\end{example}
...
\begin{itemize}
  ...
  \item $\mathit{MDiff}$ is a \emph{semantic difference} measuring how many atoms differ between the reference and foil models.
  \item $\mathit{RFDiff}$ integrates both syntactic and semantic differences, penalizing foils that diverge from the reference either in their rules or in their resulting models.
...
\begin{example}[Selecting the best contrastive explanations]\label{ex:best-contrastive-explanations}
  Recall Example~\ref{ex:example4} and the three contrastive explanations depicted in Figure~\ref{fig:example4}.
  %
  Here, depending of which criteria (i.e. cost function) we use, we may select different best contrastive explanations.
  %

  %
  First, if we use the $\mathit{PDiff}$ foil cost function, only the foil corresponding to explanation (1) would be part of $\mathit{Bestfoils}(P_r, A, R, Q, \mathit{PDiff})$ with a weight of $0$.
  % 
  Regarding, explanations (2) and (3), their respective foils will have the same weight $1$ although explanation (3) required two changes: the removal of rule $4$ and the picking of $w$ in the choice rule.
  %
  That is, we cannot decide between (2) and (3) counting differences in the programs alone.
  %
  In order to notice this latter semantic change, we need to include the \emph{reference model} $I_r$ as well in the preference by using \emph{contrast cost functions} instead.
  %

  %
  For instance, if we use the $\mathit{MDiff}$, which counts the differences among $I_r$ and $I_f$, is easy to see that contrast we obtain
  a weight of $4$ for the foil corresponding to explanation (1),
  $2$ for the foil corresponding to explanation (2),
  and $5$ corresponding to explanation (3).
  %
  Therefore $\mathrm{BestContrasts}((P_r, I_f), A, R, Q, \mathit{MDiff})$ would only include the foil leading to (2) as the preffered explanation.
  %
  Note how when using $\mathit{MDiff}$ alone, we do not penalize any addition/removal to the reference program.
  %
  
  %
  Alternatively, we can use $\mathit{RFDiff}$ instead that can be computed as the sum of the weights previously reported for the other two cost functions, that is:
  a weight of $0 + 4 = 4$ for (1),
  a weight of $1 + 2 = 3$ for (2),
  and a weight of $1 + 5 = 6$ for (3).
  %
  The ordering among foil induced by this, better matches the intuition of \emph{closeness} between the reference and foil.
  %
  In short, (2) would be the preffered explanation since the hypothetical contrastive world is closer to the reference.
  \comment{
  ...
  \comment{Brais: this notion do not capture the } 
\end{example}
