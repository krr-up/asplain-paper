...
\begin{definition}[Foil cost function]\label{def:foil-cost}
  ...
  \end{itemize}
  %
  a \emph{foil cost function} for $(P_r, A, R, Q)$ is any function
  %
  \[
...
\begin{example}[Program-difference cost function]\label{ex:program-difference-cost-function}
  ...
  %
  We define the particular foil cost function
  \[
...
\begin{definition}[Contrast cost function]\label{def:contrast-cost}
  ...
  \end{itemize}
  %
  a \emph{contrast cost function} for $((P_r, I_r), A, R, Q)$ is any function
  %
  \[
...
\begin{example}[Selecting the best contrastive explanations]\label{ex:best-contrastive-explanations}
  ...
  %
  This matches better with the intuition of 
  %
  ...
  %
  Finally, if we are still
\end{example}

\paragraph{Open questions and future}
\begin{itemize}
  \item {\color{red} Do we want to provide more functions (and examples?) as for instance preffering semantic changes over syntatic (or the other way around)?}
\end{itemize}
