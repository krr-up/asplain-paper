...
\begin{itemize}
      ...
      \item An overview of the system workflow is illustrated in Figure~?.
\end{itemize}
...
\begin{itemize}
      ...
      \comment{S: We might want to put this in an appendix}
...
\begin{itemize}
      ...
      \item To generate a model subgraph,
       ...
       where the rule in Line~\ref{model:rule} states that a rule node $R\in\Vd\cup\Vc$
       holds if
...
\begin{itemize}
    ...
    \item Since tags are treated as theory atoms, this also serves as a mean to avoid simplification when grounding,
      ...
      \item The set \removable\ can be directly obtained with rule
       \lstinline[language=clingos]{abducible(remove, R):- tag(reference, R, program(removeable))}.
    ...
    \item Some tags can't be directly added in the program, for instance,
         ...
         For this, in \asplain,
          ...
          Some built in dynamic tags included in in our implementation help easily identify facts and constraints.
\end{itemize}
